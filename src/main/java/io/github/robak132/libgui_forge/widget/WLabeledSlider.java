 ...
 * labeled sliders also support "label updaters" that can update the label when the value is changed.
 *
...
public class WLabeledSlider extends WAbstractSlider {
    ...
    public void paint(GuiGraphics context, int x, int y, int mouseX, int mouseY) {
        ...
        if (thumbState == 1 && isFocused()) {
            float px = 1 / 32f;
            ScreenDrawing.texturedRect(context, thumbX, thumbY, thumbWidth, thumbHeight, WSlider.LIGHT_TEXTURE, 24 * px,
                    0 * px, 32 * px, 20 * px, 0xFFFFFFFF);
        }
    ...
    private void drawButton(GuiGraphics context, int x, int y, int state, int width) {
        ...
        ScreenDrawing.texturedRect(context, x, y, halfWidth, 20, texture, buttonLeft, buttonTop,
                buttonLeft + buttonWidth, buttonTop + buttonHeight, 0xFFFFFFFF);
        ScreenDrawing.texturedRect(context, x + halfWidth, y, halfWidth, 20, texture, buttonEndLeft, buttonTop,
                200 * px, buttonTop + buttonHeight, 0xFFFFFFFF);
    }
    ...
    public void addNarrations(NarrationElementOutput builder) {
        if (getLabel() != null) {
            builder.add(NarratedElementType.TITLE,
                    Component.translatable(Localisation.WIDGET_LABELED_SLIDER_NARRATION_TITLE, getLabel(), value, min,
                            max));
