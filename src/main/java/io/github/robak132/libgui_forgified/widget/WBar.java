...
public class WBar extends WWidget {
     ...
     * (%d) - the first is the current value of the bar's focused field, and the second is the bar's focused maximum.
     *
    ...
    public void paint(GuiGraphics context, int x, int y, int mouseX, int mouseY) {
        ...
        switch (direction) { //anonymous blocks in this switch statement are to sandbox variables
            case UP -> {
                ...
                top -= barSize;
                if (bar != null) {
                    ScreenDrawing.texturedRect(context, left, top, getWidth(), barSize, bar.image(), bar.u1(),
                            Mth.lerp(percent, bar.v2(), bar.v1()), bar.u2(), bar.v2(), 0xFFFFFFFF);
                } else {
            ...
            case RIGHT -> {
                if (bar != null) {
                    ScreenDrawing.texturedRect(context, x, y, barSize, getHeight(), bar.image(), bar.u1(), bar.v1(),
                            Mth.lerp(percent, bar.u1(), bar.u2()), bar.v2(), 0xFFFFFFFF);
                } else {
            ...
            case DOWN -> {
                if (bar != null) {
                    ScreenDrawing.texturedRect(context, x, y, getWidth(), barSize, bar.image(), bar.u1(), bar.v1(),
                            bar.u2(), Mth.lerp(percent, bar.v1(), bar.v2()), 0xFFFFFFFF);
                } else {
            ...
            case LEFT -> {
                ...
                left -= barSize;
                if (bar != null) {
                    ScreenDrawing.texturedRect(context, left, top, barSize, getHeight(), bar.image(),
                            Mth.lerp(percent, bar.u2(), bar.u1()), bar.v1(), bar.u2(), bar.v2(), 0xFFFFFFFF);
                } else {
    ...
    /**
     * The direction of a {@link WBar}, representing where the bar will grown when its field increases.
     */
